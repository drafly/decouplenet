...
def main(config, args):
    ...
    #print(config['train'].get('start_epoch', 0)+1, total_epochs+1)
    for epoch in range(config['train'].get('start_epoch', 0)+1, total_epochs+1):
        epoch_start = time.time()
        total_loss, rgb_loss, rgbamp_loss, rgbpha_loss, raw_loss, rawamp_loss, rawpha_loss, train_time = train_one_epoch(config, model, critierion, train_dataloader, optimizer, epoch, lr_scheduler, writer)
        logger.info(f'Train: [{epoch}/{config["train"]["epochs"]}] total_loss: {total_loss:.4f} rgb_loss: {rgb_loss:.6f} raw_loss: {raw_loss:.6f} rgbamp_loss:{rgbamp_loss:.4f} rgbpha_loss:{rgbpha_loss:.4f} rawamp_loss:{rawamp_loss:.4f} rawpha_loss:{rawpha_loss:.4f} lr: {optimizer.param_groups[0]["lr"]:.6f} train_time: {train_time}')
...
def train_one_epoch(config, model, critierion, data_loader, optimizer, epoch, lr_scheduler, writer):
    ...
    for idx, data in enumerate(tqdm_loader):
        ...
        pred_rgb, pred_raw = model(input_raw)
        preds = [pred_rgb]
        preds.extend([torch.abs(torch.fft.rfft2(pred_rgb, norm='backward')), torch.angle(torch.fft.rfft2(pred_rgb, norm='backward'))])
        
        preds.append(pred_raw)
        preds.extend([torch.abs(torch.fft.rfft2(pred_raw, norm='backward')), torch.angle(torch.fft.rfft2(pred_raw, norm='backward'))])
        
        gts = [gt_rgb, torch.abs(torch.fft.rfft2(gt_rgb, norm='backward')), torch.angle(torch.fft.rfft2(gt_rgb, norm='backward')),
            gt_raw, torch.abs(torch.fft.rfft2(gt_raw, norm='backward')), torch.angle(torch.fft.rfft2(gt_raw, norm='backward'))]
        ...
        tqdm_loader.set_postfix({'rgb_l':f'{losses_meter[0].val:.6f}', 'a_rgb_l':f'{losses_meter[0].avg:.6f}',
                                'raw_l':f'{losses_meter[3].val:.6f}', 'a_raw_l':f'{losses_meter[3].avg:.6f}',
                                'rgba':f'{losses_meter[1].avg:.4f}', 'rgbp':f'{losses_meter[2].avg:.4f}',
                                'rawa':f'{losses_meter[4].avg:.4f}', 'rawp':f'{losses_meter[5].avg:.4f}',
    ...
    return [loss_meter.avg, 
            losses_meter[0].avg, losses_meter[1].avg, losses_meter[2].avg, 
            losses_meter[3].avg, losses_meter[4].avg, losses_meter[5].avg,
            train_time]
