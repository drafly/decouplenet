...
class SIDSonyDataset(data.Dataset):
    def __init__(self, data_dir, image_list_file, patch_size=None, data_type='train', 
                max_clip=1.0, min_clip=None, transpose=True,
                h_flip=True, v_flip=True, ratio=True, **kwargs):
        """
...
class SIDFujiDataset(data.Dataset):
    def __init__(self, data_dir, image_list_file, patch_size=None, data_type='train',
                max_clip=1.0, min_clip=None, transpose=True, h_flip=True, v_flip=True,
                ratio=True, **kwargs):
        assert os.path.exists(data_dir), "data_path: {} not found.".format(data_dir)
        self.data_path = data_dir
        image_list_file = os.path.join(data_dir, image_list_file)
...
if __name__=='__main__':
    ...
    dataset = SIDSonyDataset(data_dir='E:\Deep Learning\datasets\RAW\SID\Sony',
                            image_list_file='Sony_test_list.txt',data_type='test')
    #dataset = SIDFujiDataset(data_dir='E:/Deep Learning/datasets/SID/Fuji',
    #                        image_list_file='Fuji_train_list.txt',data_type='train',patch_size=384)
    data = dataset[7]
